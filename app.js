...
// Show the observer-host game board with an optional status message.
function showHostBoard(statusMsg) {
  ...
  // Hide the board timer (individual phase functions set it as needed)
...
function startGame() {
  ...
  if (state.hostPlaying) {
    ...
    showHostBoard('Game starting… waiting for players to acknowledge their roles…');
    checkAllRoleAcks();
  }
...
function beginMurdererTurn() {
  ...
  if (state.murdererPlayerId === state.playerId) {
    ...
    showHostBoard('The murderer is planning their next move…');
...
function handleAlibiPhase(data) {
  if (state.isHost && !state.hostPlaying) {
    showHostBoard('Players are writing their alibis…');
    return;
  }
...
function handleDiscussion(data) {
  if (state.isHost && !state.hostPlaying) {
    showHostBoard('Discussion phase — review alibis and talk it out…');
    return;
  }
...
function handleVotePhase(data) {
  if (state.isHost && !state.hostPlaying) {
    ...
    showHostBoard(`Voting in progress… 0 / ${total} votes received.`);
    return;
  }
